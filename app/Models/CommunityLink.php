        ...
        if ($existing) {
            if (Auth::user()->isTrusted()) {
                ...
                if ($existing->approved)
                    session()->flash('notice', 'The link already exists and it is already approved but you are not a trusted user, so it will not be updated in the list.');
                else
                    session()->flash('notice', 'The link already exists and it is pending for approval but you are not a trusted user, so it will not be updated in the list.');
            }
            return true;
        }
        return false;
